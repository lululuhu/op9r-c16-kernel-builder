...
# ============================================================
# futex_waitv Backport Patch Generator
#
# Feature:  Multi-futex Wait (futex_waitv syscall)
# Source:   Linux 5.16
# Author:   André Almeida
# Baseline: Linux 4.19
#
# futex_waitv() allows waiting on multiple futexes simultaneously,
# improving cross-platform game compatibility (Wine/Proton).
# Developed by André Almeida and merged into mainline Linux 5.16.
#
# This script:
#   1. Clones the mainline Linux repository (shallow clone)
#   2. Searches for futex_waitv-related commits
#   3. Extracts patches via git format-patch
#   4. Attempts simple sed-based adaptation for 4.19
#   5. Generates a series file
#
# futex code is relatively stable between 4.19 and 5.16, so
# adaptation difficulty is lower compared to other features.
#
...
# Search patterns for futex_waitv related commits
SEARCH_PATTERNS=(
    "futex_waitv"
    "futex: waitv"
    ...
    "futex2"
    "futex: Add futex2"
    "futex: Allow multiple futexes"
)

# Author to search for
SEARCH_AUTHOR="André Almeida"
...
# --- Pre-flight checks ---
info "============================================"
info " futex_waitv Backport Patch Generator"
info "============================================"
...
if ! git clone --depth=1 --branch "${LINUX_TAG}" "${LINUX_REPO_URL}" "${LINUX_SRC_DIR}"; then
    ...
    warn "Skipping futex_waitv patch generation."
    rm -rf "${WORK_DIR}"
    exit 0
fi
...
info "Step 2: Deepening clone history (${DEEPEN_AMOUNT} commits) to search for futex_waitv commits..."
...
# --- Step 3: Search for futex_waitv related commits ---
info "Step 3: Searching for futex_waitv related commits..."

COMMITS_FILE="${WORK_DIR}/commits.txt"
> "${COMMITS_FILE}"

# Search by commit message patterns in futex-related files
for pattern in "${SEARCH_PATTERNS[@]}"; do
    info "  Searching pattern: '${pattern}'"
    git log --oneline --no-merges --grep="${pattern}" -- kernel/futex.c include/uapi/linux/futex.h >> "${COMMITS_FILE}" 2>/dev/null || true
done

# Search by author + "futex" keyword
info "  Searching by author: '${SEARCH_AUTHOR}' (keyword: futex)"
git log --oneline --no-merges --author="${SEARCH_AUTHOR}" --grep="futex" -- kernel/futex.c include/uapi/linux/futex.h >> "${COMMITS_FILE}" 2>/dev/null || true

# Check if we found any commits
if [[ ! -s "${COMMITS_FILE}" ]]; then
    warn "No futex_waitv commits found in available history."
...
info "Found ${COMMIT_COUNT} unique futex_waitv-related commits."
echo ""
info "Commits found:"
sed 's/^/    /' "${COMMITS_FILE}"
echo ""
...
while IFS=' ' read -r _ full_hash; do
    ...
    patch_filename="$(printf '%04d' "${PATCH_NUM}")-futex-waitv-${short_hash}.patch"
...
adapt_patch() {
    ...
    sed -i 's/v5\.16/v4.19/g' "${patch_file}" 2>/dev/null || true
    sed -i 's/Linux 5\.16/Linux 4.19/g' "${patch_file}" 2>/dev/null || true

    # 2. Normalize line endings (CRLF -> LF)
    sed -i 's/\r$//' "${patch_file}" 2>/dev/null || true

    # futex code is relatively stable between 4.19 and 5.16.
    # The main differences are minor API changes that may need
    # manual adjustment, but simple sed should handle most cases.
...
if [[ -s "${SERIES_FILE}" ]]; then
    # Prepend header
    {
        echo "# futex_waitv backport patches"
        echo "# Source: Linux ${LINUX_TAG}"
        echo "# Baseline: Linux ${BASELINE_KERNEL}"
        echo "# Feature: CONFIG_FUTEX_WAITV=y (if available)"
...
echo ""
info "============================================"
info " futex_waitv Backport Generation Complete"
info "============================================"
...
warn "      futex code is relatively stable between 4.19 and 5.16,"
warn "      but some manual adaptation may still be required."
