#!/bin/bash
# ===================================================================
# generate-patches.sh — 从主线 Linux git 仓库生成 MGLRU backport 补丁
#
# MGLRU (Multi-Gen LRU) 由 Yu Zhao 开发, 核心于主线 5.18 合入, 6.1 巩固。
# 主要涉及文件: mm/vmscan.c, mm/mmzone.h, include/linux/mmzone.h,
#               mm/swap.c, mm/vmscan.h, Documentation/admin-guide/mm/multigen_lru.rst
#
# 本脚本流程:
#   1. 浅克隆 mainline linux 仓库 (torvalds) 并加深历史
#   2. 用 git log --grep "multi-gen LRU" --author "Yu Zhao" 定位提交
...
CLONE_TAG="v6.1"                       # 覆盖 5.18 核心 + 6.1 巩固
DEFAULT_TREE="$SCRIPT_DIR/.linux-mainline"
COMMIT_GREP="multi-gen LRU"
AUTHOR_GREP="Yu Zhao"
BASELINE="4.19"
SCAN_RANGE="v5.18..v6.1"               # 仅作参考/日志

info "=== MGLRU Backport Patch Generator ==="
...
prepare_source() {
    ...
    info "Deepening history (shallow-since 2020-01-01) to locate MGLRU commits..."
    git -C "$TREE" fetch --shallow-since="2020-01-01" origin "$CLONE_TAG" 2>/dev/null \
        || git -C "$TREE" fetch --unshallow 2>/dev/null \
...
# ---- Step 2: 定位 MGLRU 相关 commit ----
info "Searching for MGLRU commits (grep: '$COMMIT_GREP', author: '$AUTHOR_GREP')..."
...
if [[ ${#COMMITS[@]} -eq 0 ]]; then
    warn "No MGLRU commits found in the cloned repository."
    : > "$OUTPUT_DIR/series"
    info "Created empty series file."
    exit 0
fi
...
for c in "${COMMITS[@]}"; do
    ...
    short="${c:0:12}"
    out="$OUTPUT_DIR/${num}-mglru-${short}.patch"
...
adapt_patch() {
    ...
    if cp "$src" "$dst" 2>/dev/null; then
        # 4.19 没有 folio API。这里只做少量已知等价替换以减少噪声,
        # 远不足以让补丁干净 apply, 必须人工处理冲突。
        sed -i -E \
...
{
    echo "# MGLRU (Multi-Gen LRU) backport series for $BASELINE"
    echo "# Source: $LINUX_REPO (grep: '$COMMIT_GREP', author: '$AUTHOR_GREP')"
    echo "# Range reference: $SCAN_RANGE"
...
echo "" >&2
echo -e "${BLUE}========================================${NC}" >&2
echo -e "${BLUE}  MGLRU Patch Generation Summary${NC}" >&2
echo -e "${BLUE}========================================${NC}" >&2
...
info "Next: run apply-backports.sh --baseline $BASELINE --features mglru"
